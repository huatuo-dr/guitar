...
export const eventBeats = note => 4 / note.duration * (note.dotted ? 1.5 : 1);
...
export function attachVocals(score,vocals) {
  ...
  const bars=score.bars.map((bar,i)=>{
    ...
    if(beats!==(vocal.beats??4)) throw new Error(`第${bar.number}小节简谱时值为${beats}拍`);
    if(beats!==(bar.beats??4)) throw new Error(`第${bar.number}小节简谱与伴奏时值不一致`);
...
export function createBeatPositioner(events,vocal,beats,left,width) {
  ...
  for(const sequence of [events,vocal.events]){
    let time=0;
    for(const n of sequence){points.add(time);if(n.grace?.length)ornament.set(time,n.grace.length);if(n.dotted)dotted.add(time);time+=eventBeats(n);}
  }
...
export function renderNumberedMelody(vocal,{position,left,right,incomingTie=false}) {
  const baseline=235;
  let elapsed=0;
  const notes=vocal.events.map(note=>{
    const placed={...note,time:elapsed,x:position(elapsed+1)};elapsed+=eventBeats(note);return placed;
  });
  ...
  if(incomingTie)svg+=`<path d="M ${left} ${baseline-37} Q ${(left+notes[0].x)/2} ${baseline-40} ${notes[0].x} ${baseline-31}" fill="none" class="melody-tie"/>`;
  if(notes.at(-1).tieToNext)svg+=`<path d="M ${notes.at(-1).x} ${baseline-31} Q ${(notes.at(-1).x+right)/2} ${baseline-40} ${right} ${baseline-37}" fill="none" class="melody-tie"/>`;
