...
const url=new URL('../sheet_music/卡农指弹.html',import.meta.url).href;
const data=JSON.parse(await readFile(new URL('../score/canon.json',import.meta.url),'utf8'));
...
try{
 ...
 await page.goto(url);
 assert.equal(await page.locator('h1').innerText(),'《卡农》指弹');
 assert.match(await page.locator('.metadata').innerText(),/编配[\s\S]*深蓝雨吉他[\s\S]*C[\s\S]*60–65/);
 assert.equal(await page.locator('.measure').count(),56);assert.equal(await page.locator('.score-system').count(),14);
 assert.equal(await page.locator('#bar-14 .slide-label').textContent(),'S');
 assert.equal(await page.locator('#bar-18 .hammer').textContent(),'P');
 assert.equal(await page.locator('.lyric').count(),0);
 assert.doesNotMatch(await page.locator('.legend').innerText(),/歌词|下扫|上扫/);
 assert.deepEqual(await page.locator('.route [data-jump]').evaluateAll(nodes=>nodes.map(n=>Number(n.dataset.jump))),[1,13,17,18,22,24,28,48,52]);
 ...
 for(const rows of ['4','2']){
  ...
  assert.equal(await page.locator('.numbered-melody').count(),56);
  ...
  assert.equal(await page.locator('.score-system').count(),Math.ceil(56/Number(rows)));
 ...
 await page.screenshot({path:'artifacts/canon-desktop.png',fullPage:true});
 await page.pdf({path:'artifacts/canon-print.pdf',preferCSSPageSize:true,printBackground:true});
 ...
 await page.screenshot({path:'artifacts/canon-mobile.png',fullPage:true});
 ...
 await page.locator('.section-nav [data-jump="52"]').click();
 assert.equal(await page.locator('#bar-52').evaluate(el=>el===document.activeElement),true);
 ...
 assert.equal(await staticPage.locator('.measure').count(),56);assert.equal(await staticPage.locator('.fret').count(),expectedFrets.length);await staticContext.close();
 await page.goto(new URL('../index.html',import.meta.url).href);await page.locator('#search').fill('深蓝雨吉他');
 assert.equal(await page.locator('.score-card').count(),1);await page.locator('[data-score-id="canon-fingerstyle"] .open-score').click();assert.equal(await page.locator('h1').innerText(),'《卡农》指弹');
 console.log('PASS: 卡农56小节、全曲品数、技法、反复、简谱边界、手机、打印、离线和首页入口通过。');
