...
const url=new URL('../sheet_music/空心弹唱伴奏.html',import.meta.url).href;
try{
 ...
 await page.goto(url);
 assert.equal(await page.locator('h1').innerText(),'《空心》弹唱伴奏');
 assert.match(await page.locator('.metadata').innerText(),/光泽[\s\S]*C[\s\S]*男声 5–7 品[\s\S]*女声 0–2 品/);
 assert.equal(await page.locator('.measure').count(),47);assert.equal(await page.locator('.score-system').count(),12);
 assert.equal(await page.locator('.melody-tuplet').count(),1);
 assert.equal(await page.locator('#bar-4 .tab-tie').count(),1);
 assert.equal(await page.locator('#bar-12 .tab-tie').count(),1);
 assert.deepEqual(await page.locator('.route [data-jump]').evaluateAll(nodes=>nodes.map(n=>Number(n.dataset.jump))),[1,5,31]);
 ...
 for(const rows of ['4','2']){
  ...
  assert.equal(await page.locator('.numbered-melody').count(),47);
  for(const bar of [46,47])assert.equal(await page.locator(`#bar-${bar} .melody-tie`).count(),1,'长延音跨行保留');
  assert.equal(await page.locator('.score-system').count(),Math.ceil(47/Number(rows)));
 ...
 await page.screenshot({path:'artifacts/kongxin-desktop.png',fullPage:true});
 await page.pdf({path:'artifacts/kongxin-print.pdf',preferCSSPageSize:true,printBackground:true});
 ...
 await page.screenshot({path:'artifacts/kongxin-mobile.png',fullPage:true});
 ...
 assert.equal(await staticPage.locator('.measure').count(),47);assert.ok(await staticPage.locator('.lyric').count()>0);await staticContext.close();
 await page.goto(new URL('../index.html',import.meta.url).href);await page.locator('#search').fill('光泽');
 assert.equal(await page.locator('.score-card').count(),1);await page.locator('[data-score-id="kong-xin-accompaniment"] .open-score').click();assert.equal(await page.locator('h1').innerText(),'《空心》弹唱伴奏');
 console.log('PASS: 空心47小节、扫弦延音、三连音、歌词边界与间距、手机、打印、离线和首页入口通过。');
