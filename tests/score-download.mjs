...
const files = ['偏爱指弹.html','老男孩弹唱伴奏.html','后来弹唱伴奏.html'];
...
try {
  for (const hosted of [false,true]) {
    ...
    for (const file of files) {
      ...
      for (const rows of ['2','4']) {
        ...
        if (file !== files[0]) assert.equal(await reopened.locator('#score .measure').count(),file === files[1] ? 55 : 49);
      }
  ...
  console.log('PASS: 三份曲谱本地/网站下载、按钮位置、移动端布局、离线重开、2/4小节切换、重复下载与打印隐藏通过。');
} finally {
  await browser.close();
