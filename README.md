...
图片导出按当前完整谱 / 简易谱以及每行小节布局生成 1600 像素宽的 PNG，长谱在完整谱行之间分图，预览后逐张保存；手机可长按预览图片保存。图片包含标题、曲谱、简谱和歌词，不包含工具栏及播放高亮。导出采用离屏副本，不改变页面行数、缩放或阅读模式。`src/score-export.js` / `.css` 共用导出菜单与图片逻辑，`html-to-image 1.11.13` 及 MIT 许可内嵌到七份 HTML，不依赖网络资源。运行 `npm run test:export` 验证菜单、PDF 入口、实际 PNG、小节覆盖及手机离线使用。
