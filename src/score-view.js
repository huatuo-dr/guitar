...
function setupScoreView({storageKey, viewport, baseWidths, apply, autoScroll=true}) {
  ...
  update();
  if(autoScroll)setupScoreAutoScroll({storageKey:storageKey+':auto-scroll'});
}

// Capture a pristine standalone document before adding runtime UI.
// A Blob download also works when this page is opened directly via file://.
function setupScoreDownload({restore}={}) {
  // An early reader may already have changed layouts while audio was downloading.
  // Restore its original markup in a detached copy, keeping the complete audio assets.
  const root=restore?document.documentElement.cloneNode(true):document.documentElement;
  if(restore)restore(root);
